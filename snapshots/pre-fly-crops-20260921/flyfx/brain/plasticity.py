...
class HeadPlasticity:
    ...
    def learn(self, profit: float, side: str) -> str:
        if side not in ("BUY", "SELL") or profit == 0:
            return "plastic skip"
        return self.learn_r(float(np.tanh(profit / 400.0)), side)

    def learn_r(self, r_mult: float, side: str) -> str:
        """Three-factor DA from an R-multiple (size-invariant; used by weekly train)."""
        if side not in ("BUY", "SELL") or r_mult == 0:
            return "plastic skip"
        r = float(np.tanh(r_mult))
        self.last_r = r
