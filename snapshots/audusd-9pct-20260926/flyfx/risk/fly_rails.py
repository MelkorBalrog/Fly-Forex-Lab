...
class BookRails:
    ...
    def on_close(
        ...
        if usd <= 0:
            ...
            self.fails = 0
            self.last_loss_r = 0.0
            if int(snowball_n) > 0 or float(usd) >= float(day_lock_usd):
                self.day_lock = str(day or self.day_lock)
                if int(snowball_n) > 0:
                    self.armed_protect = True
                    notes.append("snowball lock  rest of UTC day + 1% peak protect")
                else:
                    notes.append("day lock  rest of UTC day after the win")
        return "  ".join(notes)

    def apply_latent(self, lat) -> None:
        """Circuit and pair-admit from the pair's hidden risk. Kill DD stays on the RISK profile."""
        if lat is None:
            return
        self.circuit_n = int(lat.circuit_n)
        self.circuit_bars = int(lat.circuit_bars)
        self.admit_n = int(lat.admit_n)
        self.admit_pf = float(lat.admit_pf)
        self.admit_mult = float(lat.admit_mult)
