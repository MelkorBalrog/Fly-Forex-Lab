...
from flyfx.paths import BANC_CACHE as CACHE_DIR, BANC_PROJECT as BANC_ROOT
...
class BancRiskBrain:
    ...
    def apply_reward_r(self, r_mult: float, side: str) -> str:
        kick = 0.5 if r_mult > 0 else -0.3
        if r_mult == 0:
            return "banc skip"
        if self.walk_idx.size:
            self.reward[self.walk_idx] += np.float32(kick * 0.6)
        if self.threat_idx.size:
            self.reward[self.threat_idx] += np.float32(-kick * 0.4)
        return self.plastic.learn_r(r_mult, side)
