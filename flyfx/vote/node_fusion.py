...
def confidence_drive(feat: dict, pack: dict) -> tuple[float, float]:
    """Bull and bear current for the confidence fly.

    Without an ANN, the mix is the existing robustness blend. When the
    averaged nets supply ``nn_conf`` (0..1) and ``nn_sign`` (+1 BUY, -1 SELL,
    0 HOLD), that level is part of the magnitude and the signed level is part
    of the side.
    """
    nn_raw = pack.get("nn_conf") if isinstance(pack, dict) else None
    has_nn = nn_raw is not None
    if has_nn:
        nn_c = float(np.clip(float(nn_raw), 0.0, 1.0))
        nn_sign = float(np.clip(float(pack.get("nn_sign") or 0.0), -1.0, 1.0))
        mag = float(
            np.clip(
                0.28 * float(pack.get("p_edge", 0.5))
                + 0.20 * float(pack.get("conf_proxy", 0.5))
                + 0.16 * float(pack.get("p_trend", 0.5))
                + 0.16 * float(pack.get("p_fade", 0.5))
                + 0.20 * nn_c,
                0.0,
                1.0,
            )
        ...
        side = (
            0.36 * float(pack.get("tech_fused", 0.0))
            + 0.24 * float(pack.get("trend_fused", 0.0))
            + 0.20 * float((feat or {}).get("impulse", 0.0))
            + 0.20 * (nn_c * nn_sign)
        )
    else:
        mag = float(
        ...
        side = (
            ...
            + 0.25 * float((feat or {}).get("impulse", 0.0))
        )
...
def tech_channels(feat: dict, raw_tech: str, kind: str) -> dict[str, float]:
    ...
    bounce = 0.0
    if kind == "bounce" or str(kind).startswith("fib-"):
        bounce = 0.85 * sign
    elif kind == "cont":
        bounce = 0.40 * sign
    return {
