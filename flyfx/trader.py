...
def should_flatten(
    ts: int,
    flat_hour: float = FLAT_HOUR,
    friday_flat: float = 1.0,
) -> bool:
    g = time.gmtime(ts)
    if float(friday_flat) > 0.5 and g.tm_wday == 4 and g.tm_hour >= 17:
        return True
...
def trade_loop(brain: FlyBrain, prices, args: argparse.Namespace, broker, delay: float) -> dict:
    ...
    account = AccountSim(
        ...
        leverage=getattr(args, "leverage", 50.0),
    ...
    triplet_book = None
    try:
        from flyfx.brain.triplet_ann import TripletBook

        triplet_book = TripletBook.open(
            loop_sym,
            list(getattr(args, "replay_bars", None) or []),
            load_pair_bars,
            fresh=bool(getattr(args, "nn_fresh", False)),
        )
    except Exception as exc:
        triplet_book = None
        if not quiet:
            print(f"triplet  off ({exc})")
    if triplet_book is not None:
        triplet_book.persist = not bool(getattr(args, "book_tune_eval", False))
        if not quiet:
            names = "+".join(triplet_book.legs)
            print(
                f"triplet  {loop_sym}  {names}  8 nets  "
                f"n={triplet_book.n_updates}  → indicator on every node"
            )
    last_shadow_step = -99
    ...
    def persist_state(*, force: bool = False) -> None:
        ...
        if triplet_book is not None:
            try:
                ...
                if not quiet:
                    print(f"triplet  save failed: {exc}")
        if crop_pool is not None:
    ...
    try:
        for bar in prices:
            ...
            if triplet_book is not None and stamp:
                from flyfx.brain.triplet_ann import apply_triplet

                apply_triplet(feat, triplet_book.observe(int(stamp), float(close)))
            ...
            if stop and ticket is not None:
                close_now("stop-out")
            elif ticket is not None and should_flatten(
                stamp, book.flat_hour, book.friday_flat
            ):
                ...
                if step % (15 if delay > 0 else 80) == 0:
                    ...
                    print(
                        ...
                        + f"eq ${snap['equity']:.2f}"
                        + (
                            f"  tri {float(feat.get('triplet') or 0):+.2f}"
                            if abs(float(feat.get("triplet") or 0.0)) >= 0.05
                            else ""
                        )
                    )
                ...
                if ticket is None and decision in ("BUY", "SELL") and feat["ready"]:
                    ...
                    if room and in_session(stamp, str(getattr(args, "symbol", ""))) and (
                        ...
                        if not ok:
                        ...
                        elif float(book.max_trades) > 0 and len(trades) >= int(book.max_trades):
                            print(f"[{label}] skip {decision}: book trade cap {int(book.max_trades)}")
                        elif float(book.side) > 0.5 and decision != "BUY":
                            print(f"[{label}] skip {decision}: book buys only")
                        elif float(book.side) < -0.5 and decision != "SELL":
                            print(f"[{label}] skip {decision}: book sells only")
                            ...
                            if pb_skip:
                                ...
                                boost = max(float(book.size_boost), 0.0)
                                if boost <= 0.0:
                                    boost = 1.0
                                book_mult *= boost
                                ...
                                if (
                                    ...
                                    if wider > placed_risk * 1.02 and lots > 0.0:
                                        ...
                                        placed_risk = wider
                                cost_sl = float(trade_geom["sl_atr"])
                                cost_tp = float(trade_geom["tp_atr"])
                                if float(book.wide_ok) > 0.5:
                                    if float(book.sl_atr) > 0:
                                        cost_sl = float(book.sl_atr)
                                    if float(book.tp_atr) > 0:
                                        cost_tp = float(book.tp_atr)
                    ...
                    if (
                        ...
                        and not should_flatten(stamp, book.flat_hour, book.friday_flat)
                        and not rails.kill
    ...
    persist_state(force=True)
    if triplet_book is not None and triplet_book.persist and not quiet:
        print(f"triplet  saved brains/{loop_sym}.tri.npz  n={triplet_book.n_updates}")
...
def clamp_leverage(value: object, default: float = 50.0) -> float:
    """Account leverage as the ratio, so 50 means 1:50."""
    try:
        v = float(value)
    except (TypeError, ValueError):
        v = float(default)
    if v != v:
        v = float(default)
    return float(max(1.0, min(v, 2000.0)))
...
def prepare_book(brain, bars, args, broker) -> None:
    """Load this pair's book. The tune checkbox diagnoses one replay, then stops.

    A saved return that already meets the target is reused with no extra
    replay. Otherwise the checkbox runs the saved book once. If a winner was
    closed while price kept going, it applies the trend-hold book and checks
    that once. The better of those two is saved. It does not keep searching.
    """
    import tempfile
    ...
    recorded = saved_return(sym)
    if recorded is not None and recorded + 1e-9 >= float(cfg.target_return):
        args.book_cfg = cfg
        if bool(getattr(args, "book_tune", False)):
            print(f"book-tune  {sym}  already {recorded:.1%}  reuse saved  {cfg.brief()}")
        return
    if not bool(getattr(args, "book_tune", False)):
        if load_book(sym) is None:
            save_book(sym, cfg)
        args.book_cfg = cfg
        return

    balance = float(getattr(args, "balance", 100_000.0) or 100_000.0)
    pip = float(pair_spec(sym).get("pip") or 0.0001)
    plastic = brain.dump_plastic() if hasattr(brain, "dump_plastic") else None
    print(
        f"book-tune  {sym}  8 nets read the week, then one diagnostic replay  "
        f"target {cfg.target_return:.0%}"
    )

    def evaluate(trial_cfg: BookCfg) -> dict:
        if plastic is not None:
            brain.load_plastic(plastic)
        trial = argparse.Namespace(**vars(args))
        trial.book_cfg = trial_cfg
        trial.book_tune = False
        trial.book_tune_eval = True
        trial.evolve_quiet = True
        trial.reset_adapt = True
        trial.adapt_state = str(Path(tempfile.gettempdir()) / f"fly_book_tune_{sym}.json")
        out = trade_loop(
            brain,
            replay_prices(bars, float(getattr(args, "pair_spread", 0.0) or 0.0)),
            trial,
            broker,
            delay=0.0,
        )
        net = float((out or {}).get("net_usd") or 0.0)
        print(f"book-tune  net ${net:,.0f}  {trial_cfg.brief()}")
        return out or {}

    base = evaluate(cfg)
    base_net = float(base.get("net_usd") or 0.0)
    if base_net / max(balance, 1.0) >= float(cfg.target_return):
        save_book(sym, cfg, net_usd=base_net, balance=balance)
        args.book_cfg = cfg
        if plastic is not None:
            brain.load_plastic(plastic)
        print(f"book-tune  saved brains/{sym}.book.json  met target on the saved book")
        return
    nxt = prescribe_book(cfg, list(base.get("trades") or []), bars, balance, pip)
    if asdict_same(cfg, nxt):
        save_book(sym, cfg, net_usd=base_net, balance=balance)
        args.book_cfg = cfg
        if plastic is not None:
            brain.load_plastic(plastic)
        print(f"book-tune  no winner left on the table  saved the current book  net ${base_net:,.0f}")
        return
    heads = " ".join(
        f"{name} {score:+.2f}"
        for name, score in zip(
            ("side", "hold", "stop", "one", "trail", "worth", "kill", "gate"),
            BookAdjustFleet().decide(week_vector(bars, list(base.get("trades") or []), balance, pip)),
        )
    )
    print(f"book-tune  week decision  {heads}")
    print("book-tune  check that book once")
    alt = evaluate(nxt)
    alt_net = float(alt.get("net_usd") or 0.0)
    if alt_net > base_net:
        best, best_net = nxt, alt_net
    else:
        best, best_net = cfg, base_net
    save_book(sym, best, net_usd=best_net, balance=balance)
    args.book_cfg = best
    if plastic is not None:
        brain.load_plastic(plastic)
    ret = best_net / max(balance, 1.0)
    print(f"book-tune  saved brains/{sym}.book.json  net ${best_net:,.0f}  {ret:.1%}")


def asdict_same(a, b) -> bool:
...
def bars_for_training(
    ...
    """Saved simulation history, plus the Yahoo or MetaTrader bars selected now.

    The replay window stays ``current_bars``. Training sees every continuous
    segment stored under data/history for this pair, then that window.
    The same timestamp keeps the current feed.
    """
    from flyfx.exec.history_bars import load_history, remember_bars

    tf = str(tf_name or "M5").upper()
    if current_bars:
        remember_bars(symbol, tf, current_bars, quiet=True)
    hist = load_history(symbol, tf)
    merged = merge_bars(hist, current_bars)
    note = (
        f"history data/history/{str(symbol).upper()}/{tf} n={len(hist)}  "
...
def replay_symbol(
    ...
    if not bool(getattr(args, "book_tune_eval", False)) and not bool(getattr(args, "evolve_quiet", False)):
        from flyfx.exec.history_bars import remember_bars

        remember_bars(args.symbol, tf_name, bars)
...
def _gui_apply_common(cmd: dict, args: argparse.Namespace) -> None:
    ...
    if "leverage" in cmd:
        args.leverage = clamp_leverage(cmd.get("leverage"), getattr(args, "leverage", 50.0))
    try:
...
def _gui_flag_snapshot(args: argparse.Namespace) -> dict:
    ...
    snap["leverage"] = clamp_leverage(getattr(args, "leverage", 50.0))
...
def run(args: argparse.Namespace) -> None:
    ...
    args.leverage = clamp_leverage(getattr(args, "leverage", 50.0))
    if data_file:
        args.replay = True
    ...
    if args.replay or hst_path is not None or pair_list:
        ...
        for i, sym in enumerate(symbols):
            ...
            if not bool(getattr(args, "book_tune_eval", False)) and not bool(getattr(args, "evolve_quiet", False)):
                ...
                remember_bars(sym, used_name, bars)
            print(
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--leverage", type=float, default=50.0, help="account leverage, e.g. 50 = 1:50")
    ...
    p.add_argument(
        ...
        default=False,
        dest="book_tune",
        help="before Replay, diagnose this pair once. If a winner was closed while "
        "price kept going, apply the trend-hold book and check it once. A pair "
        "already at its saved return target is reused. At most two quiet replays. "
        "Off reuses brains/{PAIR}.book.json.",
    )
    p.add_argument(
        "--profit-recycle",
        action=argparse.BooleanOptionalAction,
