...
flowchart TB
  ...
  subgraph RISK["I  Size  —  confidence scales lots"]
    ...
    BANC["female BANC cluster net<br/>+ MaleCNS ol_sensory / vnc_sensory<br/>risk_mult on lots / snowball / bounce"]
  ...
  subgraph LEARN["K  Feedback"]
    PNL["close P&amp;L"]
    DA["PAM/PPL kick into all heads"]
    PL["three-factor plasticity<br/>Δreadout · Δgains  (W frozen)"]
    TAX["estimate_tax"]
    STATE["adapt_state.json<br/>trail/SL + fusion + plastic + BANC"]
  end
  ...
  C23 -->|"BUY or SELL + tag"| GATE --> BANC --> SIZE --> SIM --> BRO
  BRO -->|"orders"| MT4
  SIM --> EXIT --> PNL
  PNL --> DA --> PL
  PNL --> GATE
...
flowchart LR
  ...
  DA["PAM/PPL 340<br/>kick + three-factor plasticity"] --> XT
  DA --> XF
  DA --> XC
```

BANC has **no retina**. Risk is mapped onto female neck clusters (threat / walking / tactile / feeding) from `BANC-project-main` v888 CSVs, mixed with MaleCNS `ol_sensory` (smell) and `vnc_sensory` (touch). That multiplier scales lots, can block a weak bounce, and can refuse a snowball. It does **not** veto a 2oo3. Full ~188k BANC edgelist streams from CAVE if `CAVE_TOKEN` is set; without a token the cluster net is the live path.
...
| Female risk | BANC cluster + MaleCNS VNC/olfactory | `BancRiskBrain` | `banc_risk.py` |
| Dopamine RL | Eligibility × DA on readout/gains | `HeadPlasticity` | `plasticity.py` |
...
| Snowball | Pyramid into winners paid by float | `snowball_add_lots`, `AccountSim.add` | `fly_forex.py`, `account_sim.py` |
| Dashboard | Live 3-head lab view | `--gui` | `fly_dash.py`, `web/dash.html` |
...
**Bar loop.** Update indicators → quotes → if in session and not CHOP (or already in a trade): run **trend + fade** heads, fuse each node, run **fly-conf**, `gate_vote`, `committee`. If flat and BUY/SELL: `gate` → `size_lots` → open. If in a trade: trail / BE from the **original** entry, then **snowball** if fly-trend still agrees and floating P&L can pay for the add, then flatten / stop.
...
**Snowball (default on).** At most two adds, each ~55% of the first slice. Requires fly-trend on-side, no fade veto, regime still with the trade, profit past BE but **not yet trailing**, a small pullback from the extreme, floating USD ≥ 1.50× the add’s stop risk, and BANC ≥ 0.85. Trail stays on the first fill. `--no-snowball` restores single-slice sizing.

**Recovery martingale (default on, fly-trader rails).** After a *scratch* 2oo3 loss (< 0.45R), next size is ×1.20, then ×1.28, then stop. Full ATR stops, 1oo3 bounces, pair-admit, BANC < 0.90, or 2% drawdown: no double. Kill switch at 2.5% from peak (1.0% after a snowball win). A snowball winner locks new entries until the next UTC day so the pyramid is not given back. A pair starts at 0.60× until 2 trades with PF ≥ 1. `--no-martingale` turns recovery off; snowball still presses winners.

**Both sides.** Tech fires BUY continuation in UP (RSI 58–68) and SELL continuation in DOWN (RSI 32–42), plus pullback bounces both ways. A winning trade no longer locks the book to one direction.

**Learning loop.** Close updates Beta win-rate, Normal win/loss pips, calibration bins, scorer weights, trail/SL/TP (not RSI / min-impulse), each node's Beta `p_win`, a **PAM/PPL dopamine pulse** into all heads, and a **three-factor plasticity** step: eligibility traces on descending activity × reward nudge the descending readout and bull/bear sensory gains. Connectome `W` stays frozen (measured synapses). Female BANC (plus MaleCNS smell/touch) writes `risk_mult` into `adapt_state.json` with fusion + plastic. `--reset-adapt` starts from priors. `--no-fusion` skips the gate (restore `snapshots/2oo3-swarm-20260919`). `--no-plastic` / `--no-banc` freeze those layers. `--no-snowball` restores single-slice sizing. `--gui` opens `http://127.0.0.1:8765/`. `--overfit` replays the majors on the EURUSD window.
