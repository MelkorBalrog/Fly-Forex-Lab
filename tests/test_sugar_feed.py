...
def test_full_winner_is_left_for_the_trail():
    out = mix_appetite(
        0.70,
        0.90,
        tag="2oo3",
...
def test_cli_sugar_off_by_default(monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["fly_forex.py", "--replay", "--sugar", "--fly-crops"])
    crops = parse_args()
    assert crops.fly_crops is True
    assert _sugar_enabled(crops) is True
    monkeypatch.setattr(sys, "argv", ["fly_forex.py", "--replay"])
    bare = parse_args()
    assert bare.fly_crops is False
