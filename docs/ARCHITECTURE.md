...
### Sugar feed (eat / don't eat)

On by default. The 23 MaleCNS **sugar GRNs** are driven with “food present” (2oo3/3oo3, BANC calm, Kalman clean). After a short pass we **read** their activity = feeding. A separate **satiety tank** (equity vs peak, day's gain, floating profit, trades today) is internal sugar — not painted on the eyes. Appetite = feeding × hunger. High appetite sizes and allows snowball. Full + weak food skips the open. Full + a winner flattens (`sugar full`). Never changes the 2oo3 side. `--no-sugar` / uncheck GUI **sugar feed** / MT4 `InpSugar=false`. Use `--legacy-2oo3 --no-sugar --risk-tol balanced` to match the freeze book.
...
The dashboard shows: winner, vote type, confidence, inhibited list, losers, and any conflict string.

---

## 7. After the pick — size, snowball, exit

Same plumbing as before. The committee tag is still `2oo3` / `3oo3` (optionally `+flow`), so `SetupScorer.gate` does not need a new enum.
...
| Sugar feed | 23 GRNs + satiety tank (size/skip/snowball/spit-out) | `vote_sugar`, `mix_appetite` | `flyfx/brain/sugar_feed.py` |
...
**Weekly dopamine train (other pairs).** `python fly_train.py` walks Yahoo/HST M5 *before* the EURUSD eval week and writes `brains/{PAIR}.json`. The GUI **Train DA** button does the same for one pair from the DATA file / path (or HST/Yahoo). EURUSD stays factory-frozen (`snapshots/eurusd-snowball-lock-20260919`) unless you check **train EURUSD** / `--force-brain`. Replay does **not** load those files unless **use DA brain** / `--use-brain`.
