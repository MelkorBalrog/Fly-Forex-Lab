...
| **Replay sim** | Fast historical sim. No MT4. No live orders. With **recalibrate** on (default), walk-forward retunes before the book. Check **use evolved** to load `brains/{PAIR}.evo.json`. Check **use DA brain** to load PAM/PPL traces. |
...
**Tune book to 7%**. Each pair has its own book file `brains/{PAIR}.book.json`. Replay loads that file and starts the simulation. It does not run a search loop first. A different pair keeps a different file.
