"""After a win: wait for Bollinger squeeze→expand, then size from profit only.

Rule (operator request):
- After a sealed profit of ``W`` dollars, block new entries until Bollinger
  bandwidth first *closes* (squeeze vs its MA) and then *starts to open*
  (expansion).
- The next trade may use at most ``1.5 × W`` as its money base
  (profit + 50% of that profit from the original balance). Example: W=$1,000
  → budget $1,500 — not the full account.
- A losing close while armed clears the gate and budget (back to full size).
"""
...
class ProfitRecycleGuard:
    ...
    def on_close(self, usd: float) -> str:
        ...
        if pnl > 0:
            ...
            self._squeeze_bw = 0.0
            note = (
                f"profit-recycle  arm  win ${pnl:,.0f}  "
                f"next budget ${self.budget:,.0f} ({self.mult:.2f}×)  wait BB squeeze→expand"
            )
            self.last = RecycleSnap(
                active=True,
                phase=self.phase,
    ...
    def size_budget(self) -> float | None:
        """Full account. A 1.5× cap after the first win sized the rest of the book away."""
        return None
