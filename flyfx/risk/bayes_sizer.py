...
class BayesianSizer:
    ...
    def risk_scale(self, base_risk: float) -> tuple[float, str]:
        """Scale base risk% by half-Kelly and P(edge>0). Factory clip is [0.35, 1.00]."""
        from flyfx.risk.latent import FACTORY, bayes_scale_from_parts

        if base_risk <= 0:
            return 0.0, "fixed"
        lat = self.latent or FACTORY
        post = self.posterior()
        half = 0.5 * post["kelly"]
        parts = {
            "kelly_raw": half / float(lat.kelly_div),
            "edge": float(lat.edge_base) + float(lat.edge_gain) * float(post["p_edge"]),
            "rr_raw": float(post["payoff"]) / float(lat.rr_div),
        }
...
class SetupScorer:
    ...
    def gate(self, scored: dict, tag: str) -> tuple[bool, str, float]:
        ...
        ok, note, mult = hidden_size(scored, tag, self.latent, threshold=self.threshold)
        if not ok:
            self.skips += 1
            self.skip_reasons.append(note)
            if "need 2oo3" in note and len(self.skip_reasons) > 40:
                self.skip_reasons = self.skip_reasons[-40:]
            return False, note, 0.0
        return True, note, mult
