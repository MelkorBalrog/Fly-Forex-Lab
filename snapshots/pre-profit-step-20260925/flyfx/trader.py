...
def trade_loop(brain: FlyBrain, prices, args: argparse.Namespace, broker, delay: float) -> dict:
    ...
    if not quiet:
        if recycle_on:
            print(
                f"recycle  on  after win: wait BB squeeze→expand, "
                f"next trade money ≤ {recycle.mult:.2f}× profit "
                f"(e.g. $1k win → ${1000 * recycle.mult:,.0f} budget)"
            )
        else:
            print("recycle  off")
            ...
            if stop and ticket is not None:
                ...
                if ticket is None and decision in ("BUY", "SELL") and feat["ready"]:
                    ...
                    bank_why = bank_guard.observe(
                        ...
                        hold_risk=bool(getattr(args, "hold_risk", True)),
                    ...
                    if hit_sl:
                        ...
                        bars_held >= int(round(g["min_hold_bars"]))
                        and trend_flip
                        and not open_widened
                    ):
                        close_now("trend flip")
                    elif (
                        bars_held >= int(round(g["min_hold_bars"])) + 2
                        and chopped
                        and not open_widened
                    ):
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=True,
        dest="profit_recycle",
        help="after a win: wait for Bollinger squeeze→expand before re-entry, and size the "
        "next trade from ≤1.5× that profit (profit + 50%%). --no-profit-recycle disables.",
    )
    p.add_argument(
        "--profit-recycle-mult",
        type=float,
