...
How the pieces connect, including **two MaleCNS fly instances** on one shared wiring diagram and a **2-out-of-3 committee**. Python owns the strategy; MetaTrader 4 is only I/O.
...
flowchart TB
  ...
  subgraph SWARM["E  FlySwarm  —  one W, two brains"]
  ...
  subgraph ALGOS["F  Parallel algorithms"]
    TECH["tech  PullbackSetup<br/>bounce or continuation"]
    FLOW["flow_algo<br/>Kalman regime voter"]
  end

  subgraph VOTE["G  Committee"]
    C23["2oo3  {tech, fly-trend, fly-fade}<br/>opposite fly = veto<br/>flow sizes up, cannot open alone"]
  end

  subgraph RISK["H  Size  —  confidence scales lots"]
  ...
  subgraph EXEC["I  Execution"]
    SIM["AccountSim"]
    BRO["PaperBroker or OrderSend"]
    EXIT["trail / BE / SL / TP / flatten"]
  end

  subgraph LEARN["J  Feedback"]
    PNL["close P&amp;L"]
    DA["DA reward into both heads"]
    TAX["estimate_tax"]
...
Timing still comes from **tech**. The two flies and tech vote. **flow** is a fourth parallel algorithm that only changes size.
...
flowchart TB
  ...
  C -->|"any fly votes opposite"| VETO["HOLD  veto"]
  C -->|"tech HOLD"| NONE["HOLD  no-tech"]
...
## 3. Connectome — two instances, one graph

`W` is loaded once. Each head has its own state `x` and dopamine `r`. Same descending readout, different retina drive.
...
flowchart LR
  ...
  XT --> VT["BUY / SELL / HOLD"]
  XF --> VF["BUY / SELL / HOLD"]
  DA["PAM/PPL 340<br/>kick both heads on close"] --> XT
  DA --> XF
```
...
| `tech` | PullbackSetup | committee | Timing (bounce vs cont) |
| `fly-trend` / `fly-fade` | FlySwarm | committee | Diversity + veto |
...
**Bar loop.** Update indicators → quotes → if flat, in session, not CHOP: run **both** fly heads + tech + flow → `committee` → if BUY/SELL, `gate` → `size_lots` → open. In a trade: trail / BE / flatten / stop.

**Connectome loop.** Two 10-step leaky-tanh passes on the **same** `W`. Trend uses `τ×0.55` (votes more). Fade uses full `τ` (vetoes only when sure). DA from the last close is deposited on **both** `r` vectors.

**Learning loop.** Close updates Beta win-rate, Normal win/loss pips, calibration bins, scorer weights, trail/SL/TP (not RSI / min-impulse), and `reports/adapt_state.json`. `--reset-adapt` starts from priors.
...
Demo / sim only. Two copies of a real fly wiring diagram plus 2oo3 voting are a filter — not a guarantee of live profit.
