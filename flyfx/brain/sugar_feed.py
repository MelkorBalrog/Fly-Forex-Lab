...
def food_nutrition(
    ...
    *,
    ...
    if in_trade and trend_with:
        tag_score = 0.88
    elif "3oo3" in t or "3oo4" in t or "4oo4" in t or n_agree >= 3:
        tag_score = 1.0
    elif "2oo3" in t or "2oo4" in t or n_agree >= 2:
        tag_score = 0.74
    elif "bounce" in t:
        tag_score = 0.46
    elif in_trade:
...
def mix_appetite(
    ...
    *,
    ...
    if sat >= 0.92:
        ...
        skip = True
        reason = "no appetite"
    # Satiety may skip a new entry. It does not block a pyramid, and it does
    # not close a winner: the trail and the high-peak giveback do that.
    # Spitting at ~1 ATR was banking the EURUSD continuation before the add.
    snowball = True
    flatten = False
