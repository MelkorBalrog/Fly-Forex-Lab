...
class FlyBrain:
    ...
    def encode_conf(self, feat: dict, pack: dict) -> np.ndarray:
...
class FlySwarm:
    """MaleCNS instances sharing W: trend, fade/reversion, and confidence."""
    ...
    def vote_conf(self, feat: dict, pack: dict, substeps: int) -> tuple[str, float]:
        vote, score, _, _ = self.core.run_window_state(
            self.conf_x, self.core.encode_conf(feat, pack), self.conf_r, substeps, tau_scale=0.70
        )
        return vote, score
...
def trade_loop(brain: FlyBrain, prices, args: argparse.Namespace, broker, delay: float) -> None:
    ...
    print(
        "committee  2oo3  {tech, fly-trend, fly-fade}   parallel: flow-algo   "
        + (
    ...
    def close_now(reason: str) -> None:
        ...
        save_adapt_state(state_path, bayes, scorer, params, extra={"fusion": board.dump()})
        print(
    ...
    try:
        for bar in prices:
            ...
            if fusion_on and need_fly:
                conf_fly, _ = swarm.vote_conf(feat, pack, args.substeps)
