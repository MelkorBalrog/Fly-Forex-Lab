...
from flyfx.paths import CONFIG_DIR, DASH_HTML as HTML_PATH, PROJECT_ROOT
...
# Built-in lab preset matching the operator screenshot (scalp + AI + selective FUSE).
DEFAULT_LAB_CONFIG: dict = {
    "name": "default",
    "symbol": "EURUSD",
    "timeframe": "M5",
    "bars": 2000,
...
def _safe_config_name(name: str) -> str:
    ...
    if text.startswith("_"):
        inner = "".join(ch if ch.isalnum() or ch in "-_" else "_" for ch in text[1:])
        inner = inner.strip("._-")[:62] or "cfg"
        return "_" + inner
...
def load_config_file(name: str) -> dict | None:
    key = str(name or "").strip().lower()
    if key in ("default", "lab-default", "builtin"):
        return dict(DEFAULT_LAB_CONFIG)
    if key in ("_last", "last"):
        path = CONFIG_DIR / "_last.json"
    else:
        path = CONFIG_DIR / f"{_safe_config_name(name)}.json"
    if not path.exists():
        return None
    try:
...
def save_config_file(name: str, payload: dict) -> Path:
    CONFIG_DIR.mkdir(parents=True, exist_ok=True)
    safe = _safe_config_name(name)
    path = CONFIG_DIR / f"{safe}.json"
    blob = dict(payload or {})
    blob["name"] = safe
    blob["saved_at"] = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())
    path.write_text(json.dumps(blob, indent=2), encoding="utf-8")
    # Always mirror the most recent save for auto-restore.
    (CONFIG_DIR / "_last.json").write_text(json.dumps(blob, indent=2), encoding="utf-8")
    return path
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_GET(self) -> None:  # noqa: N802
                ...
                if path.startswith("/api/configs/"):
                    ...
                    self._send(
                        200,
                        json.dumps({"ok": True, "config": cfg}).encode("utf-8"),
            ...
            def do_POST(self) -> None:  # noqa: N802
                ...
                if path == "/api/configs":
                    action = str(cmd.get("action") or "save").lower()
                    if action == "delete":
                        name = _safe_config_name(cmd.get("name") or "")
                        target = CONFIG_DIR / f"{name}.json"
                        if name.startswith("_") or not target.exists():
                            self._send(400, b'{"ok":false,"error":"cannot delete"}', "application/json")
                            return
                        try:
                            target.unlink()
                        ...
                        self._send(
                            200,
                            json.dumps({"ok": True, "names": list_saved_configs()}).encode(),
                            "application/json; charset=utf-8",
                        )
                        return
                    name = cmd.get("name") or "config"
                    payload = cmd.get("config") if isinstance(cmd.get("config"), dict) else cmd
                    try:
                        saved = save_config_file(str(name), payload)
                    except OSError as exc:
                        self._send(
                            500,
                            json.dumps({"ok": False, "error": str(exc)}).encode(),
                        ...
                        json.dumps(
                            {
                                "ok": True,
                                "name": saved.stem,
                                "path": str(saved.relative_to(PROJECT_ROOT)).replace("\\", "/"),
                                "names": list_saved_configs(),
                            }
                        ).encode(),
                        "application/json; charset=utf-8",
