...
def worth_budget(budget: float, equity: float, impulse_abs: float) -> float:
    """Raise a 1.5× profit budget toward the account when impulse is strong.

    At 0.92 the floor is 70% of equity. At 1.0 it is the whole account.
    The margin cap still limits lots. A milder impulse stays on 1.5×.
    """
    base = float(budget)
    eq = float(equity)
    mag = float(impulse_abs)
    if mag < WORTH_IMPULSE or eq <= base:
        return base
    t = min(1.0, (mag - WORTH_IMPULSE) / (1.0 - WORTH_IMPULSE))
    frac = 0.70 + 0.30 * t
...
class ProfitReflex:
    ...
    def lift_budget(
        ...
        tape = (
            worth_budget(float(budget), float(equity or 0.0), abs(impulse))
            if with_trade
            else float(budget)
        )
        if tape > raised:
